...
. Review the Dockerfile. This is a systemctl based dockerfile.  There is work being done so that the _wants_ files don't have to be removed.  At some point there will be a minimal systemd environment for containers. 
+
----
# cat Dockerfile 
FROM fedora:20
...
This section shows how to use hostnames and link to an existing container.  Issue the _docker run_ command and link to the _mariadb_ container.
